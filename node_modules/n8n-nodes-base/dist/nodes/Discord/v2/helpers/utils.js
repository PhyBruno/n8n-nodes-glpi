...
function prepareEmbeds(embeds, i = 0) {
    return embeds
        .map((embed, index) => {
        let embedReturnData = {};
        ...
        if (!embedReturnData.description) {
            throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Description is required, embed ${index} in item ${i} is missing it`);
        }
...
async function checkAccessToChannel(channelId, userGuilds, itemIndex = 0) {
    ...
    if (!guildId) {
        throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Could not fing server for channel with the id ${channelId}`, {
            itemIndex,
        });
    }
